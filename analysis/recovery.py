...
def main():
    ...
    print("Last 14 days:")
    print(f"{'date':>10} {'km':>5} {'HRV':>4} {'RHR':>4}  status")
    recent = w.tail(14)
    for d, r in recent.iterrows():
        ...
        if pd.notna(r["hrv"]) and r["hrv"] < lo:
            flag = " ⚠ below baseline"
        print(f"{d:%m-%d} {k:9.1f} {r['hrv']:4.0f} {r['rhr']:4.0f}  "
              f"{str(r['status']).lower()}{flag}")

    weekly_km = km.resample("W").sum()
    weekly_hrv = w["hrv"].resample("W").mean()
    weekly_rhr = w["rhr"].resample("W").mean()
    print(f"\n{'week ending':>12} {'km':>6} {'HRV':>5} {'RHR':>5}  load")
    ...
    if cur_hrv >= base_hrv and cur_rhr <= base_rhr + 1:
        print("Verdict: absorbing the load — HRV holding, RHR stable.")
    else:
